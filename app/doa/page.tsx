...
import { 
  Search, ArrowLeft, Sparkles, ChevronRight, Folder, 
...
const databaseKategori: Kategori[] = [
  ...
  { 
    id: "w9", 
    nama: "Wirid Bakda Shalat Fardhu", 
    tipe: "wirid", 
    jumlah: 1, 
    list: [
      { 
...
وَإِلٰهُكُمْ إِلٰهٌ وَاحِدٌ لَا إِلٰهَ إِلَّا هُوَ الرَّحْمٰنُ الرَّحِيْمُ
...
export default function DoaPage() {
  ...
  const toggleFavorite = (judul: string, e?: React.MouseEvent) => {
    ...
    const favBaru = favorites.includes(judul) 
  ...
  const listDoaUntukTampil = kategoriAktif 
    ? kategoriAktif.list.filter(doa => doa.judul.toLowerCase().includes(kataKunci.toLowerCase()))
    : activeTab === "favorit" 
      ? daftarFavorit.filter(doa => doa.judul.toLowerCase().includes(kataKunci.toLowerCase()))
      : [];
